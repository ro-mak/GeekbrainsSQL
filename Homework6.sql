...
UPDATE messages SET from_user_id = 16 WHERE to_user_id = 14 LIMIT 10;

UPDATE messages SET from_user_id = 12 WHERE to_user_id = 14 LIMIT 7;

UPDATE messages SET from_user_id = 15 WHERE to_user_id = 14 LIMIT 5;

SELECT COUNT(from_user_id) as number_of_messages, from_user_id, to_user_id FROM messages WHERE to_user_id  = 14 GROUP BY from_user_id ORDER BY number_of_messages DESC LIMIT 1;

SELECT COUNT(*) as likes_tolessthan10_year_old FROM likes, profiles, media, users WHERE likes.media_id = media.id AND media.user_id = profiles.user_id AND profiles.user_id = users.id AND (YEAR(DATE(NOW())) - YEAR(profiles.birthday)) < 10;

UPDATE profiles SET gender = "f" WHERE user_id = 12;

SELECT COUNT(*) as number_of_likes, profiles.gender FROM likes, profiles WHERE likes.user_id = profiles.user_id GROUP BY profiles.gender ORDER BY number_of_likes DESC LIMIT 1;
